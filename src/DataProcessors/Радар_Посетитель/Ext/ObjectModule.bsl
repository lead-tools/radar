...
Процедура ПосетитьМодульМенеджера(ОбъектМетаданных, Окружение, СвойстваМенеджера = Неопределено)
	...
	ПосетитьМодуль(ОбъектМетаданных.ПолноеИмя() + ".МодульМенеджера", Окружение, СвойстваМенеджера);
...
Процедура ПосетитьЗначенияПеречисления(ЗначенияПеречисления, СвойстваМенеджера)
	...
	Для Каждого ОбъектЗначениеПеречисления Из ЗначенияПеречисления Цикл
		...
		Если СвойстваМенеджера <> Неопределено Тогда
			СвойстваМенеджера.Добавить(ОбъектЗначениеПеречисления.Имя);
		КонецЕсли;
	КонецЦикла; 
...
Процедура ПосетитьОбъектПланВидовРасчета(ОбъектПланВидовРасчета) 
	...
	СвойстваОбъекта = Новый Массив;
	СвойстваМенеджера = Новый Массив;
	...
	ПосетитьПредопределенныеЭлементы(ОбъектПланВидовРасчета, СвойстваМенеджера);
	
	ПосетитьМодульОбъекта(ОбъектПланВидовРасчета, "ОкружениеОбъектаПланаВидовРасчета", СвойстваОбъекта);
	ПосетитьМодульМенеджера(ОбъектПланВидовРасчета, "ОкружениеМенеджераПланаВидовРасчета", СвойстваМенеджера);
...
Процедура ПосетитьОбъектПланВидовХарактеристик(ОбъектПланВидовХарактеристик) 
	...
	ПосетитьПредопределенныеЭлементы(ОбъектПланВидовХарактеристик, СвойстваМенеджера);
	
	ПосетитьМодульОбъекта(ОбъектПланВидовХарактеристик, "ОкружениеОбъектаПланаВидовХарактеристик", СвойстваОбъекта);
	ПосетитьМодульМенеджера(ОбъектПланВидовХарактеристик, "ОкружениеМенеджераПланаВидовХарактеристик", СвойстваМенеджера);
...
Процедура ПосетитьОбъектПланСчетов(ОбъектПланСчетов) 
	...
	ПосетитьПредопределенныеЭлементы(ОбъектПланСчетов, СвойстваМенеджера);
	
	ПосетитьМодульОбъекта(ОбъектПланСчетов, "ОкружениеОбъектаПланаСчетов", СвойстваОбъекта);
	ПосетитьМодульМенеджера(ОбъектПланСчетов, "ОкружениеМенеджераПланаСчетов", СвойстваМенеджера);
...
Процедура ПосетитьОбъектСправочник(ОбъектСправочник) 
	...
	ПосетитьПредопределенныеЭлементы(ОбъектСправочник, СвойстваМенеджера);
	
	ПосетитьМодульОбъекта(ОбъектСправочник, "ОкружениеОбъектаСправочника", СвойстваОбъекта);
	ПосетитьМодульМенеджера(ОбъектСправочник, "ОкружениеМенеджераСправочника", СвойстваМенеджера);
...
Процедура ПосетитьПредопределенныеЭлементы(ОбъектМетаданных, СвойстваМенеджера)
	
	Если ТекущийПроект.ВнешняяКонфигурация Тогда
		
		ПредопределенныеЭлементыОбъекта = ПредопределенныеЭлементыОбъекта(ОбъектМетаданных);
		
		Для Каждого ПредопределенныйЭлемент Из ПредопределенныеЭлементыОбъекта Цикл
			СвойстваМенеджера.Добавить(ПредопределенныйЭлемент.Имя);	
		КонецЦикла; 
	...
	Иначе
		...
		Запрос = Новый Запрос;
		Запрос.Текст = СтрЗаменить(ШаблонЗапроса, "&Таблица", ОбъектМетаданных.ПолноеИмя());
		
		Выборка = Запрос.Выполнить().Выбрать();
		
		Пока Выборка.Следующий() Цикл
			СвойстваМенеджера.Добавить(Выборка.ИмяПредопределенныхДанных);
		КонецЦикла;
...
Функция ПредопределенныеЭлементыОбъекта(ОбъектМетаданных)
	
	ДобавитьНаСтекПути("Ext\Predefined.xml", Перечисления.ФорматыВыгрузки.XML);
	ДобавитьНаСтекПути(ОбъектМетаданных.Имя + ".mdo", Перечисления.ФорматыВыгрузки.EDT);
	Путь = ПутьСтрокой();
	УдалитьСоСтекаПути();
	
	Если ТекущийФорматВыгрузки = Перечисления.ФорматыВыгрузки.XML Тогда
		Возврат ПредопределенныеЭлементыОбъектаXML(Путь);
	ИначеЕсли ТекущийФорматВыгрузки = Перечисления.ФорматыВыгрузки.EDT Тогда
		Возврат ПредопределенныеЭлементыОбъектаEDT(Путь);
	Иначе
...
Функция ПредопределенныеЭлементыОбъектаXML(Путь)
	...
	Если Файл.Существует() Тогда
		...
		ЧтениеXML.Прочитать();
		Инвариант(ЧтениеXML.Имя = "PredefinedData");
			...
			Если ЧтениеXML.Имя = "Item" Тогда
				...
				Пока ЧтениеXML.Прочитать() Цикл
					Если ЧтениеXML.Имя = "Item" И ЧтениеXML.ТипУзла = ТипУзлаXML.КонецЭлемента Тогда
						Прервать;
					ИначеЕсли ИмяПредопределенного = Неопределено
						И ЧтениеXML.Имя = "Name"
						И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента
						И ЧтениеXML.Прочитать()
						И ЧтениеXML.Имя = "#text" Тогда
						ИмяПредопределенного = ЧтениеXML.Значение;
						Инвариант(ИмяПредопределенного <> Неопределено);
						ПредопределенныйЭлемент = Радар_Типы.ПредопределенныйЭлемент(ИмяПредопределенного);
						ПредопределенныеЭлементы.Добавить(ПредопределенныйЭлемент);
					КонецЕсли; 
				КонецЦикла; 
				Инвариант(ЧтениеXML.ТипУзла = ТипУзлаXML.КонецЭлемента);
			ИначеЕсли ЧтениеXML.Имя = "PredefinedData" Тогда
				Инвариант(ЧтениеXML.ТипУзла = ТипУзлаXML.КонецЭлемента);
			Иначе
	...
	Возврат ПредопределенныеЭлементы;
	
КонецФункции 

Функция ПредопределенныеЭлементыОбъектаEDT(Путь)
	...
	Если Файл.Существует() Тогда
		...
		ЧтениеXML.ОткрытьФайл(Путь);
		ЧтениеXML.Прочитать();
		Инвариант(СтрНачинаетсяС(ЧтениеXML.Имя, "mdclass:"));
		
		Пока ЧтениеXML.Прочитать() И ЧтениеXML.Имя <> "predefined" Цикл
			ЧтениеXML.Пропустить();
			Инвариант(ЧтениеXML.ТипУзла = ТипУзлаXML.КонецЭлемента);
		КонецЦикла;
		
		Если ЧтениеXML.Имя = "predefined" Тогда
			
			Пока ЧтениеXML.Прочитать() Цикл
				 
				Если ЧтениеXML.Имя = "items" Тогда
					ИмяПредопределенного = Неопределено;
					Пока ЧтениеXML.Прочитать() Цикл
						Если ЧтениеXML.Имя = "items" И ЧтениеXML.ТипУзла = ТипУзлаXML.КонецЭлемента Тогда
							Прервать;
						ИначеЕсли ИмяПредопределенного = Неопределено
							И ЧтениеXML.Имя = "name"
							И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента
