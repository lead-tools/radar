...
Процедура ПосетитьКонфигурацию(Отчет) Экспорт
	...
	КонецЦикла; 
	ХэшПроекта = ПолучитьHexСтрокуИзДвоичныхДанных(ХешированиеДанных.ХешСумма);
	
	ХэшСистемыИзменился = Ложь;
	
	ЗаписьХэшСуммы = РегистрыСведений.Радар_ХешСуммы.Получить(Новый Структура("Проект", ТекущийПроект));
	
	Если ХэшПроекта <> ЗаписьХэшСуммы.ХешСумма Тогда
		ХэшСистемыИзменился = Истина;
		ЗаписьХэшСуммы = РегистрыСведений.Радар_ХешСуммы.СоздатьМенеджерЗаписи();
		ЗаписьХэшСуммы.Проект = ТекущийПроект;
		ЗаписьХэшСуммы.ХешСумма = ХэшПроекта;
		ЗаписьХэшСуммы.Записать(Истина);
	КонецЕсли; 
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ХэшСистемыИзменился", ХэшСистемыИзменился);
...
Процедура ПосетитьМодульМенеджера(ОбъектМетаданных, Окружение)
	...
	ПосетитьМодуль(ОбъектМетаданных.ПолноеИмя() + ".МодульМенеджера", Окружение);
...
Процедура ПосетитьОбъектПеречисление(ОбъектПеречисление) 
	...
	ПосетитьЗначенияПеречисления(ОбъектПеречисление.ЗначенияПеречисления);
	ПосетитьМакеты(ОбъектПеречисление.Макеты);
	ПосетитьКоманды(ОбъектПеречисление.Команды);
	ПосетитьФормы(ОбъектПеречисление.Формы);
			
	ПосетитьМодульМенеджера(ОбъектПеречисление, "ОкружениеМенеджераПеречисления");
...
Процедура ПосетитьЗначенияПеречисления(ЗначенияПеречисления)
